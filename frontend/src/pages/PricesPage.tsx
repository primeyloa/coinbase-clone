...
interface CryptoAsset {
  ...
  change24h: number
  createdAt: string
  color?: string // optional for frontend
}
...
export default function PricesPage() {
  ...
  useEffect(() => {
    const fetchCryptos = async () => {
      try {
        ...
        // Add default colors for display
        const colors = ['#F7931A', '#627EEA', '#9945FF', '#0033AD', '#00AAE4', '#E6007A']
        const cryptosWithColors = data.map((crypto, idx) => ({
          ...crypto,
          color: colors[idx % colors.length]
        }))
        setCryptoAssets(cryptosWithColors)
      } catch (err) {
  ...
  if (error) {
    return (
      <div className="container-page py-10">
        <div className="text-center text-cb-red">{error}</div>
      </div>
    )
  }
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-8">
        <div className="bg-white border border-cb-border rounded-2xl p-5">
          ...
          <div className="flex flex-col gap-3">
            {gainers.map(a => (
              <Link to={`/trade?asset=${a._id}`} key={a._id} className="flex items-center justify-between hover:opacity-80 transition-opacity">
